...
namespace WindowsFormsApplication1
{
    class CMatriculaUPC
    {
        ...
        {
            CCurso curso_encontrado = Cursos.Find( delegate(CCurso value) { return value.Codigo == cod_curso;} );
            if (curso_encontrado != null)
            {
                curso_encontrado.Alumnos.Add(alumno);
            }
            if (!ListaAlumnos.Exists(delegate (CAlumno value) { return value.TIU == alumno.TIU; })) 
                ListaAlumnos.Add(alumno);
        }
        public List<CAlumno> ListarAlumnosDeCurso(string codcurso)
        {
            CCurso curso_encontrado = Cursos.Find(delegate (CCurso value) { return value.Codigo == codcurso; });
                if (curso_encontrado != null)
        ...
        {
            return Cursos.Exists(delegate (CCurso value) { return value.Codigo == codcurso;});
        }
        public List<CCurso> ListarCursosQueLlevaUnAlumno(int tiu)
        {
            List<CCurso> cursos_matriculados = new List<CCurso>();
            foreach (CCurso curso in Cursos)
                if (curso.Alumnos.Exists(delegate (CAlumno value) { return value.TIU == tiu; }))
        ...
        public CAlumno DameAlumnoConNumeroTIUMayor()
        {
